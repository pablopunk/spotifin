...
`scripts/release.sh x.y.z` refuses a dirty tree, tags `vX.Y.Z` if missing, and pushes the branch and tag. The Release workflow then builds every platform and creates the GitHub release with generated notes. The release does not exist until that run succeeds, so the real notes are edited in afterwards.
...
1. While CI builds, gather what ships: `git log --oneline vPREV..vNEW`. Read the commits, not only the messages, for user-facing changes.
